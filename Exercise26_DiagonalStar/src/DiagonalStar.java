public class DiagonalStar {
        ...
        if(number < 5) {
            ...
            for(int row = 1; row <= number; row++) {
                for(int col = 1; col <= number; col++){
                    if((row == 1) || (row == number) || (col == 1) || (col == number) || (row == col) ||
                            (col == (number - row + 1))) {
                        System.out.print("*");
                    }
                    else{
                ...
                System.out.println(); // To "move" to another line
            }
        }
    ...
    * When the column number equals rowCount - currentRow + 1 (where currentRow is current row number)
    ...
    NOTE: The method printSquareStar should be defined as public static like we have been doing so far in
        the course.
